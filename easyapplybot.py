...
class EasyApplyBot:
    ...
    def fill_out_phone_number(self):
        ...
        input_field = self.browser.find_element(
            "xpath", "//input[contains(@id,'phoneNumber')]")
    ...
    def send_resume(self) -> bool:
        ...
        try:
            ...
            resume_locator = (By.XPATH, '//p[contains(@class, "input-error")]')
            ...
            while True:
                ...
                buttons: list = [next_locater, review_locater, follow_locator,
                                 submit_locater, submit_application_locator, resume_locator]
                for i, button_locator in enumerate(buttons):
                    if is_present(button_locator):
                        button: None = self.wait.until(
                            EC.element_to_be_clickable(button_locator))
                    if is_present(resume_locator):
                        self.fill_resume()
                        print("Located invalid resume text")
                    if is_present(error_locator):
                        for element in self.browser.find_elements(error_locator[0],
                                                                  error_locator[1]):
                            try:
                                text = element.text
                                if ("Please enter a valid answer" in text and self.checked_invalid):
                                    self.fill_invalids()
                                    button = None
                                    break
                                elif ("Please enter a valid answer" in text and not self.checked_invalid):
                                    self.fill_invalids()
                                    # break
                            except Exception as e:
                                continue
    ...
    def fill_invalids(self):
        ...
        try:
            check_inputs = self.browser.find_elements(
                By.XPATH, '//input[contains(@value, "Yes")]')
            for input in check_inputs:
                self.browser.execute_script("arguments[0].click();", input)
                # input.click()
        except Exception as e:
            print(e)
            log.info("checkbox error")

        # try:
        #     check_input = self.browser.find_elements(
        #         By.CSS_SELECTOR, '[data-test-text-selectable-option__input=Yes]')
        #     if check_input:
        #         check_input[0].click()
        # except Exception as e:
        #     log.info("checkbox error")
        ...
        try:
            select_inputs = self.browser.find_elements(
                By.XPATH, '//option[contains(@value, "Yes")]')
            for input in select_inputs:
                input.click()
        except Exception as e:
            log.info("Select box")
        text_inputs = self.browser.find_elements(
            By.XPATH, '//input[contains(@class, "fb-dash-form-element")]')
    ...
    def fill_resume(self):
        ...
        resume_input = self.browser.find_elements(
            By.XPATH, "//label[contains(@class, 'jobs-document-upload-redesign-card__toggle-label')]")
        if resume_input:
            print(resume_input)
            resume_input[1].click()
        else:
            print("No resume input found")
    ...
    def next_jobs_page(self, position, location, jobs_per_page):
        self.browser.get(
            "https://www.linkedin.com/jobs/search/?f_LF=f_AL&f_WT=2&keywords=" +
